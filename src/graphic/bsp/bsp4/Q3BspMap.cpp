...
{ 
  ...
  GLuint vertexPosAttr = 0;//glGetAttribLocation(shader.id, "inVertex");
  GLuint texCoordAttr  = 1;//glGetAttribLocation(shader.id, "inTexCoord");
  GLuint lmapCoordAttr = 2;//glGetAttribLocation(shader.id, "inTexCoordLightmap");
...
bool Q3BspMap::ClusterVisible(int cameraCluster, int testCluster) const
{
  ...
  unsigned int idx = (cameraCluster * visData.sz_vecs) + (testCluster >> 3);

  // std::cout << "Array size: " << idx << " / " << sizeof(visData.vecs) << std::endl;
...
void Q3BspMap::CalculateVisibleFaces(mt::vec3 _cameraPosition)
{
  const Math::Vector3f cameraPosition(_cameraPosition.x, _cameraPosition.y, _cameraPosition.z);
  ...
  for (const auto &f : faces)
  {
    ...
    if (m_textures[f.texture] == NULL)
    {
      ...
      if (m_textures[f.texture] == NULL)
      {
        ...
        std::cout << "[WARNING] " << sstream.str() << std::endl;
      }
    }
  }
}
...
void Q3BspMap::RenderFace(int idx)
{
  ...
  if (m_textures[faces[idx].texture]) {
    ...
    m_textures[faces[idx].texture]->bind();
  }
  else
  {
...
void Q3BspMap::RenderPatch(int idx)
{
  ...
  if (m_textures[m_patches[idx]->textureIdx])
    m_textures[m_patches[idx]->textureIdx]->bind();
