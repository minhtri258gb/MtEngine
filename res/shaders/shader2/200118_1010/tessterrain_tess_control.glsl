...
//	float dist = df - distance(eyePos, vec3(tc_position[gl_InvocationID].x, height, tc_position[gl_InvocationID].y));
//	dist /= df;
//	dist = pow(dist, 5);
//
...
//	tessfact = max(2, tessfact);
	...
	gl_TessLevelOuter[0] = getTessFactO(0); // f
	gl_TessLevelOuter[1] = getTessFactO(1); // l
	gl_TessLevelOuter[2] = getTessFactO(2); // b
	gl_TessLevelOuter[3] = getTessFactO(3); // r
